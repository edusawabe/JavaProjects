	...
	private void abrirTimer(Event evt){
		if(primaryStageTimer == null){
			...
			if(!primaryStageTimer.isShowing()){
				...
				primaryStageTimer.requestFocus();
			}
		}
	}
...
/*		hidePainelInferior = 0;
	...
	private void btListaRebuyConsolidadoAction(Event evt){
		...
		try {
			...
			primaryStageTimer.setScene(scene);
			primaryStageTimer.setTitle("Lista de Rebuys Consolidada");
			rebuysConsolidadoController.getOlRebuysConsolidado().add(new RebuysConsolidadoLine());
			rebuysConsolidadoController.getOlRebuysConsolidado().get(0).setNome("Teste");
			rebuysConsolidadoController.getOlRebuysConsolidado().get(0).setRebuys("2");
			rebuysConsolidadoController.getOlRebuysConsolidado().get(0).setPago(false);
			primaryStageTimer.show();
			} catch(Exception e){
				e.printStackTrace();
			}
